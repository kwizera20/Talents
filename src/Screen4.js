...
import Records from '../screen4.json';
...
const renderRecord =(record, idx) =>{
    ...
    return(
        <div onClick={() => toggleRecordSelect(idx)} class="card p-6" id="card" style={{width: '250px', margin: 'auto', marginLeft:'30px', marginBottom:"30px",height:'250px'}} 
         key={record.id} >
      <img class="card-img-top text-center " style={{width: '100px', height:'100px',marginTop:'25px',marginLeft:'60px'}} src={record.icon} alt="icon" />
      <div class="card-body">
        <p class="card-text text-center"  style={{fontSize: '20px', fontWeight:"bolder"}}>{record.value}</p>
      </div>
...
const submit = () =>{
    ...
    let valueOfScreen4 = selectedRecords.join(',')
    alert ("selected records:" +valueOfScreen4);
    const valueOfScreen2=location.state.screen2;
    const valueOfScreen3=location.state.valueOfScreen3;
    navigate("/Screen5", {state:{ valueOfScreen4,valueOfScreen2,valueOfScreen3 }})
